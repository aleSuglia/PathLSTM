    ...
    public NetworkOptions(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "-hidden1": {
                    hidden1 = Integer.valueOf(args[++i]);
                    break;
                }
                case "-hidden2": {
                    hidden2 = Integer.valueOf(args[++i]);
                    break;
                }
                case "-data": {
                    datatype = args[++i];
                ...
                case "-gates":
                    ...
                    break;
                case "-dropout":
                    dropout = Float.valueOf(args[++i]);
                    break;
                case "-weightinit":
                    max = Float.valueOf(args[++i]);
                    break;
                case "-delaysupp":
                    supplementEpoch = Integer.valueOf(args[++i]);
                    break;
                case "-trainsize":
                    trainsize = Integer.valueOf(args[++i]);
                    break;
                case "-updatealpha":
                    updatealpha = true;
                    break;
                ...
                case "-ablate":
                    ...
                    break;
                default:
                    throw new Exception("ERROR! No such argument: " + args[i]);
            }
        }
    }
}
